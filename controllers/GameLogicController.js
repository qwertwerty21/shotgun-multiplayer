...
module.exports = {
  ...
  addJoiningPlayer(joinerSocket, roomID) {
    ...
    currentGameRoom.players.push({
      name: joinerSocket.name,
      id: joinerSocket.id
    });
  ...
  startGame(roomObj) {
    console.log(
      'STARTING THE GAME IN THE CONTROLLER. HERS THE DATA THE CONTROLLER RECEIVED',
      roomObj
    );
  ...
  updatePlayerMove(playerObj, roomObj) {
    ...
    if (currentGameRoom) {
      let curPlayerIndex = this.findPlayerIndexByObjKeyInArray(
        currentGameRoom.playersAlive,
        'id',
        playerObj.id
      );
      ...
      console.log(
        'THIS IS THE CURRENTGAMEROOM Before updateing the player move',
        currentGameRoom
      );
      console.log(
        'this is the player obj with crrent move received set to true',
        updatedPlayerObj
      );
      ...
      if (
        currentGameRoom.movesMadeThisRound >=
        currentGameRoom.playersAlive.length
      ) {
  ...
  updateBlocks(roomObj) {
    ...
    roomObj.playersAlive.forEach((curVal, index, array) => {
      if (curVal.currentMove && curVal.currentMove.move === 'block') {
        curVal.blocked = true;
        blockResults.push(
          `${curVal.name} Blocked! They are invulnerable during this Round.`
        );
      }
    });
  ...
  updateReloads(roomObj) {
    ...
    roomObj.playersAlive.forEach((curVal, index, array) => {
      if (curVal.currentMove && curVal.currentMove.move === 'reload') {
        curVal.bullets++;
        reloadResults.push(
          `${curVal.name} Reloaded! They now have ${
            curVal.bullets
          } total Bullets.`
        );
      }
  ...
  updateShoots(roomObj) {
    ...
    roomObj.playersAlive.forEach((curVal, index, array) => {
      if (curVal.currentMove && curVal.currentMove.move === 'shoot') {
        ...
        let targetIndex = outerThis.findPlayerIndexByObjKeyInArray(
          roomObj.playersAlive,
          'id',
          curVal.currentMove.target
        );
        if (targetIndex < 0) {
          targetIndex = outerThis.findPlayerIndexByObjKeyInArray(
            roomObj.playersDead,
            'id',
            curVal.currentMove.target
          );
          shootResults.push(
            `${curVal.name} tried to shoot ${
              roomObj.playersDead[targetIndex].name
          ...
          if (curVal.bullets > 0) {
            ...
            if (!roomObj.playersAlive[targetIndex].blocked) {
              shootResults.push(
                `${curVal.name} shot ${
                  roomObj.playersAlive[targetIndex].name
                } dead!`
              );
              //if target curretmove is shoot hold off on splicing them
              if (
                roomObj.playersAlive[targetIndex].currentMove &&
                roomObj.playersAlive[targetIndex].currentMove.move ===
                  'shoot' &&
                roomObj.playersAlive[targetIndex].bullets > 0
              ) {
                console.log('pushing to delaying kill', targetIndex);
                playersToKillOffDelayed.push(
                  roomObj.playersAlive[targetIndex].id
                );
              } else {
                //end if
    ...
    for (let i = 0; i < playersToKillOffDelayed.length; i++) {
      let delayedDeadPlayerIndex = this.findPlayerIndexByObjKeyInArray(
        roomObj.playersAlive,
        'id',
        playersToKillOffDelayed[i]
      );
      let delayedDeadPlayer = roomObj.playersAlive.splice(
        delayedDeadPlayerIndex,
        1
      );
      console.log(
        'heres roomObj after killing delayed player splice',
        roomObj.playersAlive
      );
  ...
  isRoomStillPlaying(roomObj) {
    ...
    if (roomObj.playersAlive.length <= 1) {
      ...
      if (roomObj.playersAlive[0] && roomObj.playersAlive[0].name) {
        roomObj.results.push(
          `${roomObj.playersAlive[0].name} is the final Player left standing. ${
            roomObj.playersAlive[0].name
          } wins the game!`
        );
      } else {
        roomObj.results.push(
          'Everyone is dead. There are no winners and you all suck.'
        );
      }
  ...
  prepareNextRound(playerObj, roomObj) {
    ...
    if (curPlayerIndex < 0) {
      ...
      curPlayerIndex = this.findPlayerIndexByObjKeyInArray(
        currentGameRoom.playersDead,
        'id',
        playerObj.id
      );
      //reset player object
      updatedPlayerObj = Object.assign(
        {},
        currentGameRoom.playersDead[curPlayerIndex],
        {
      ...
      updatedPlayerObj = Object.assign(
        {},
        currentGameRoom.playersAlive[curPlayerIndex],
        {
          currentMoveReceived: false,
          currentMove: null,
          blocked: false
    ...
    if (
      currentGameRoom.startNextRoundVotes >= currentGameRoom.playersAlive.length
    ) {
  ...
  resetRoomForNextRound(roomObj) {
    ...
    console.log(
      'heres currentGaemRoom The one in server memeory',
      gameRooms[roomObj.id]
    );
