    ...
    socket.on('action', function(action){
        ...
        switch(action.type){
            ...
            case 'HOST_GAME':
                ...
                console.log('emitting HOST_GAME_SERVER to client')
                //return the game room
                return socket.emit('action', {type: 'HOST_GAME_SERVER', payload: room})
            ...
            case 'JOIN_GAME': 
                ...
                if(doesRoomExistAndWantPlayers){
                    ...
                    socket.emit('action', {type: 'SET_CURRENT_PLAYER_SERVER', payload: {id: socket.id, name:action.payload.name }});
                    //emit to all sockets in the room that a new player has joined
                    return app.io.in(roomID).emit('action', {type: 'JOIN_GAME_SERVER', payload: updatedRoom}); 
                }
                //if room doesnt exist send error to client
                    ...
                    return socket.emit('action', {type: 'JOIN_GAME_FAIL_SERVER', payload: errMsg });
                }
            ...
            case 'START_GAME':
                ...
                return app.io.in(action.payload.room.id).emit('action', {type: 'START_GAME_SERVER', payload: gameState});
                //pass back to clients in room
            
            case 'SEND_CURRENT_GAME_STATE':
                ...
                if(updatedRoomWithPlayerMove.results && updatedRoomWithPlayerMove.results.length > 0){
                    ...
                    return app.io.in(updatedRoomWithPlayerMove.id).emit('action', {type: 'SHOW_ROUND_RESULTS_SERVER', payload: updatedRoomWithPlayerMove});
                }
                else{
                    ...
                    return socket.emit('action', {type: 'SEND_CURRENT_GAME_STATE_SERVER', payload: updatedGameState});
                }
            ...
            case 'START_NEXT_ROUND':
                ...
                socket.emit('action', {type: 'RESET_NEXT_ROUND_CURRENT_PLAYER_SERVER', payload: updatedGameState});
                //run throuhg room obj players alive currentmove null
                if( nextRoundRoomWithPlayerMove.room.movesMadeThisRound === 0 ){

                    return app.io.in(nextRoundRoomWithPlayerMove.room.id).emit('action', {type: 'START_NEXT_ROUND_SERVER', payload: nextRoundRoomWithPlayerMove.room});
                }
            ...
            case 'ALERT_LEAVER': 
                ...
                return app.io.in(leaverRoomID).emit('action', {type: 'ALERT_LEAVER_SERVER', payload: leaverErrMsg});
