...
app.io.on('connection', socket => {
    ...
    console.log(`socket connected. id: ${socket.id}info ${socketConnections[socket.id].info}`);
    ...
    socket.on('action', action => {
        ...
        switch(action.type){
            ...
            case constants.HOST_GAME:
                ...
                console.log('emitting HOST_GAME_SERVER to client', constants.HOST_GAME_SERVER)
                //return the game room
                return socket.emit('action', {type: constants.HOST_GAME_SERVER, payload: room})
            ...
            case constants.JOIN_GAME: 
                ...
                let doesRoomExistAndWantPlayers = controller.gamelogic.doesRoomExistAndWantPlayers(roomID);

                if(doesRoomExistAndWantPlayers){

                    let updatedRoom = controller.gamelogic.addJoiningPlayer(socket, roomID)

                    //have player join gameroom
                    socket.join(roomID)
                    
                    //set the current player only for the socket
                    socket.emit('action', {type: constants.SET_CURRENT_PLAYER_SERVER, payload: {id: socket.id, name:action.payload.name }});
                    //emit to all sockets in the room that a new player has joined
                    return app.io.in(roomID).emit('action', {type: constants.JOIN_GAME_SERVER, payload: updatedRoom}); 
                }
                //if room doesnt exist send error to client
                    ...
                    return socket.emit('action', {type: constants.JOIN_GAME_FAIL_SERVER, payload: errMsg });
                }
            ...
            case constants.START_GAME:
                ...
                return app.io.in(action.payload.room.id).emit('action', {type: constants.START_GAME_SERVER, payload: gameState});
                //pass back to clients in room
            
            case constants.SEND_CURRENT_GAME_STATE:
                console.log('CAUGHT SEND_CURRENT_GAME_STATE ACTION ON SOCKET JS SERVER asdfsdafsdafds',action.payload.game.room);

                let updatedRoomWithPlayerMove = controller.gamelogic.updatePlayerMove(action.payload.currentPlayer, action.payload.game.room)
                ...
                if(updatedRoomWithPlayerMove && updatedRoomWithPlayerMove.results && updatedRoomWithPlayerMove.results.length > 0){
                    ...
                    return app.io.in(updatedRoomWithPlayerMove.id).emit('action', {type: constants.SHOW_ROUND_RESULTS_SERVER, payload: updatedRoomWithPlayerMove});
                }
                else{
                    ...
                    return socket.emit('action', {type: constants.SEND_CURRENT_GAME_STATE_SERVER, payload: updatedGameState});
                }

            case constants.START_NEXT_ROUND:
                console.log('CAUGHT START NEXT ROUND ACTION ON SOCKET JS SERVER',action);

                let nextRoundRoomWithPlayerMove = controller.gamelogic.prepareNextRound(action.payload.currentPlayer, action.payload.game.room)
                ...
                socket.emit('action', {type: constants.RESET_NEXT_ROUND_CURRENT_PLAYER_SERVER, payload: updatedGameState});
                //run throuhg room obj players alive currentmove null
                if( nextRoundRoomWithPlayerMove.room.movesMadeThisRound === 0 ){

                    return app.io.in(nextRoundRoomWithPlayerMove.room.id).emit('action', {type: constants.START_NEXT_ROUND_SERVER, payload: nextRoundRoomWithPlayerMove.room});
                }
            ...
            case constants.ALERT_LEAVER: 
                ...
                return app.io.in(leaverRoomID).emit('action', {type: constants.ALERT_LEAVER_SERVER, payload: leaverErrMsg});
